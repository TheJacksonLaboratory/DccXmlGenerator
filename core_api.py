...
def restful_update(status_message,experiment_name,exp_ls):
    ...
    password = mycfg['corepfs_database']['password']
    loginUrl =  mycfg['corepfs_database']['loginUrl']
    updateUrl = mycfg['corepfs_database']['updateUrl']
    
    tenant = "PROD" if "PROD" in mycfg['corepfs_database']['baseURL'] else "TEST" # Right now looks only for PROD or TEST  
...
if __name__ == '__main__':
    ...
    # This is the main function. It is used to test the functions in this module.
    mycfg = cfg.parse_config(path="config.yml")
    baseURL = mycfg['corepfs_database']['baseURL']
    username = mycfg['corepfs_database']['username']
    password = mycfg['corepfs_database']['password']
    exp_ls = ['KGTT49']
    restful_update('Review Completed','KOMP_GLUCOSE_TOLERANCE_TEST_EXPERIMENT',exp_ls)
    """
    my_auth = HTTPBasicAuth(username, password)
